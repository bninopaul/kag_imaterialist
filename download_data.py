...
if __name__ == '__main__':
    ...
    dataset, outdir, num_images = sys.argv[1:]
    print(dataset, outdir, num_images)
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    # parse json dataset file
    if num_images=="None":
        fnames_urls = parse_dataset(dataset, outdir, None)
    elif type(int(num_images))==int:
        fnames_urls = parse_dataset(dataset, outdir, int(num_images))
